 ...
 * Page class
 */
class SentTblGrid extends SentTbl
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SentTblGrid";
    ...
    public $FormName = "fsent_tblgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "SentTblGrid";
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_datetime_sent") && $CurrentForm->hasValue("o_datetime_sent") && $this->datetime_sent->CurrentValue != $this->datetime_sent->DefaultValue) {
            return false;
        }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'datetime_sent' first before field var 'x_datetime_sent'
        $val = $CurrentForm->hasValue("datetime_sent") ? $CurrentForm->getValue("datetime_sent") : $CurrentForm->getValue("x_datetime_sent");
        if (!$this->datetime_sent->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->datetime_sent->Visible = false; // Disable update for API request
            } else {
                $this->datetime_sent->setFormValue($val, true, $validate);
            }
            $this->datetime_sent->CurrentValue = UnFormatDateTime($this->datetime_sent->CurrentValue, $this->datetime_sent->formatPattern());
        }
        if ($CurrentForm->hasValue("o_datetime_sent")) {
            $this->datetime_sent->setOldValue($CurrentForm->getValue("o_datetime_sent"));
        }

        // Check field name 'fk_id_message' first before field var 'x_fk_id_message'
        $val = $CurrentForm->hasValue("fk_id_message") ? $CurrentForm->getValue("fk_id_message") : $CurrentForm->getValue("x_fk_id_message");
        if (!$this->fk_id_message->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->fk_id_message->Visible = false; // Disable update for API request
            } else {
                $this->fk_id_message->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_fk_id_message")) {
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->id_sent->CurrentValue = $this->id_sent->FormValue;
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->datetime_sent->EditValue = HtmlEncode(FormatDateTime($this->datetime_sent->CurrentValue, $this->datetime_sent->formatPattern()));
            ...
            if ($this->fk_id_message->getSessionValue() != "") {
                ...
                $this->fk_id_message->OldValue = $this->fk_id_message->CurrentValue;
                $this->fk_id_message->ViewValue = $this->fk_id_message->CurrentValue;
                $curVal = strval($this->fk_id_message->CurrentValue);
                if ($curVal != "") {
                    $this->fk_id_message->ViewValue = $this->fk_id_message->lookupCacheOption($curVal);
                ...
                $this->fk_id_message->EditValue = HtmlEncode($this->fk_id_message->CurrentValue);
                $curVal = strval($this->fk_id_message->CurrentValue);
                if ($curVal != "") {
                    $this->fk_id_message->EditValue = $this->fk_id_message->lookupCacheOption($curVal);
                    if ($this->fk_id_message->EditValue === null) { // Lookup from database
                        $filterWrk = SearchFilter("[id_message]", "=", $curVal, DATATYPE_NUMBER, "");
                        $sqlWrk = $this->fk_id_message->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                        $conn = Conn();
                        $config = $conn->getConfiguration();
                        ...
                        if ($ari > 0) { // Lookup values found
                            $arwrk = $this->fk_id_message->Lookup->renderViewRow($rswrk[0]);
                            $this->fk_id_message->EditValue = $this->fk_id_message->displayValue($arwrk);
                        } else {
                            $this->fk_id_message->EditValue = HtmlEncode(FormatNumber($this->fk_id_message->CurrentValue, $this->fk_id_message->formatPattern()));
                        }
                    }
                } else {
                    $this->fk_id_message->EditValue = null;
                }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->datetime_sent->Required) {
            if (!$this->datetime_sent->IsDetailKey && EmptyValue($this->datetime_sent->FormValue)) {
                $this->datetime_sent->addErrorMessage(str_replace("%s", $this->datetime_sent->caption(), $this->datetime_sent->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->datetime_sent->FormValue, $this->datetime_sent->formatPattern())) {
            $this->datetime_sent->addErrorMessage($this->datetime_sent->getErrorMessage(false));
        }
        if ($this->fk_id_message->Required) {
            if (!$this->fk_id_message->IsDetailKey && EmptyValue($this->fk_id_message->FormValue)) {
                $this->fk_id_message->addErrorMessage(str_replace("%s", $this->fk_id_message->caption(), $this->fk_id_message->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // datetime_sent
        $this->datetime_sent->setDbValueDef($rsnew, UnFormatDateTime($this->datetime_sent->CurrentValue, $this->datetime_sent->formatPattern()), CurrentDate(), $this->datetime_sent->ReadOnly);

        // fk_id_message
        if ($this->fk_id_message->getSessionValue() != "") {
            $this->fk_id_message->ReadOnly = true;
        }
        ...
        // Check referential integrity for master table 'message_tbl'
        $detailKeys = [];
        $keyValue = $rsnew['fk_id_message'] ?? $rsold['fk_id_message'];
        $detailKeys['fk_id_message'] = $keyValue;
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "message_tbl", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "message_tbl") {
            $this->fk_id_message->CurrentValue = $this->fk_id_message->getSessionValue();
        }
        ...
        // datetime_sent
        $this->datetime_sent->setDbValueDef($rsnew, UnFormatDateTime($this->datetime_sent->CurrentValue, $this->datetime_sent->formatPattern()), CurrentDate(), false);
        ...
        // Check referential integrity for master table 'sent_tbl'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["fk_id_message"] = $this->fk_id_message->CurrentValue;
        $masterTable = Container("message_tbl");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
    ...
    protected function setupMasterParms()
    {
        ...
        if ($masterTblVar == "message_tbl") {
            $masterTbl = Container("message_tbl");
            $this->fk_id_message->Visible = false;
