...
/**
 * Table class for sent_tbl
 */
class SentTbl extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // id_sent
        $this->id_sent = new DbField(
            $this, // Table
            'x_id_sent', // Variable name
            'id_sent', // Name
            '[id_sent]', // Expression
            'CAST([id_sent] AS NVARCHAR)', // Basic search expression
            3, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '[id_sent]', // Virtual expression
            false, // Is virtual
        ...
        $this->fk_id_message = new DbField(
            $this, // Table
            'x_fk_id_message', // Variable name
            'fk_id_message', // Name
            '[fk_id_message]', // Expression
            'CAST([fk_id_message] AS NVARCHAR)', // Basic search expression
            3, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '[fk_id_message]', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->twiliocode_sent = new DbField(
            $this, // Table
            'x_twiliocode_sent', // Variable name
            'twiliocode_sent', // Name
            '[twiliocode_sent]', // Expression
            '[twiliocode_sent]', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '[twiliocode_sent]', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->twiliocode_sent->InputTextType = "text";
        $this->twiliocode_sent->Nullable = false; // NOT NULL field
        $this->twiliocode_sent->Required = true; // Required field
        $this->twiliocode_sent->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['twiliocode_sent'] = &$this->twiliocode_sent;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[sent_tbl]";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id_sent->setDbValue($conn->lastInsertId());
            $rs['id_sent'] = $this->id_sent->DbValue;
        }
        return $success;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['id_sent']) && !EmptyValue($this->id_sent->CurrentValue)) {
                $rs['id_sent'] = $this->id_sent->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('id_sent', $rs)) {
                AddFilter($where, QuotedName('id_sent', $this->Dbid) . '=' . QuotedValue($rs['id_sent'], $this->id_sent->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->id_sent->CurrentValue : $this->id_sent->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_sent->CurrentValue = $keys[0];
            } else {
                $this->id_sent->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_sent', $row) ? $row['id_sent'] : null;
        } else {
            $val = !EmptyValue($this->id_sent->OldValue) && !$current ? $this->id_sent->OldValue : $this->id_sent->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_sent@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SentTblList");
    }
    ...
    {
        global $Language;
        if ($pageName == "SentTblView") {
            return $Language->phrase("View");
        } elseif ($pageName == "SentTblEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SentTblAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SentTblView", $parm);
        } else {
            $url = $this->keyUrl("SentTblView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SentTblList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SentTblList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("SentTblDelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_sent\":" . JsonEncode($this->id_sent->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_sent->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_sent->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("id_sent") ?? Route("id_sent")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->id_sent->CurrentValue = $key;
            } else {
                $this->id_sent->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->twiliocode_sent->EditValue = $this->twiliocode_sent->CurrentValue;
