...
<form name="fcontact_tbladdopt" id="fcontact_tbladdopt" class="ew-form" action="<?= HtmlEncode(GetUrl(Config("API_URL"))) ?>" method="post" novalidate autocomplete="on">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="<?= Config("API_ACTION_NAME") ?>" id="<?= Config("API_ACTION_NAME") ?>" value="<?= Config("API_ADD_ACTION") ?>">
<input type="hidden" name="<?= Config("API_OBJECT_NAME") ?>" id="<?= Config("API_OBJECT_NAME") ?>" value="contact_tbl">
<input type="hidden" name="addopt" id="addopt" value="1">
<?php if ($Page->name_contact->Visible) { // name_contact ?>
    <div<?= $Page->name_contact->rowAttributes() ?>>
        <label class="col-sm-2 col-form-label ew-label" for="x_name_contact"><?= $Page->name_contact->caption() ?><?= $Page->name_contact->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="col-sm-10"><div<?= $Page->name_contact->cellAttributes() ?>>
...
<?php if ($Page->phone_contact->Visible) { // phone_contact ?>
    <div<?= $Page->phone_contact->rowAttributes() ?>>
        <label class="col-sm-2 col-form-label ew-label" for="x_phone_contact"><?= $Page->phone_contact->caption() ?><?= $Page->phone_contact->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="col-sm-10"><div<?= $Page->phone_contact->cellAttributes() ?>>
