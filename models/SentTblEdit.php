 ...
 * Page class
 */
class SentTblEdit extends SentTbl
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SentTblEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "SentTblEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_sent") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_sent->setQueryStringValue($keyValue);
                $this->id_sent->setOldValue($this->id_sent->QueryStringValue);
            } elseif (Post("id_sent") !== null) {
                $this->id_sent->setFormValue(Post("id_sent"));
                $this->id_sent->setOldValue($this->id_sent->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("id_sent") ?? Route("id_sent")) !== null) {
                    ...
                    $this->id_sent->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("SentTblList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "SentTblList") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "SentTblList") {
                            ...
                            $returnUrl = "SentTblList"; // Return list page content
                        }
                    }
    ...
    public function renderRow()
    {
        ...
        // id_sent
        $this->id_sent->RowCssClass = "row";

        // fk_id_message
        $this->fk_id_message->RowCssClass = "row";

        // datetime_sent
        $this->datetime_sent->RowCssClass = "row";

        // twiliocode_sent
        $this->twiliocode_sent->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->id_sent->Required) {
            if (!$this->id_sent->IsDetailKey && EmptyValue($this->id_sent->FormValue)) {
                $this->id_sent->addErrorMessage(str_replace("%s", $this->id_sent->caption(), $this->id_sent->RequiredErrorMessage));
            }
        }
